...
dlblocks() {
    ...
    /usr/bin/wget --quiet "https://seed.blkcc.xyz/acc/block_log" -O "${DATADIR}/witness/blockchain/block_log"
...
updatewit() {
    do_update() {
        ...
        "${DIR}/scripts/python/update_witness.py" update "${user}" "${active_privkey}" --publicownerkey "${owner_pubkey}" --blocksize 131072 --url "https://condenser.blkcc.xyz/@${user}" --creationfee "0.100 EFTG" --interestrate 0
    }
